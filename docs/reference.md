...
Where prumo prints a `-> suggestion`, that is almost certainly the intended file; the two usually differ only in `-` versus `_`. With no suggestion, the target was renamed or deleted, so update the link or drop it. For a markdown link, git's history is asked the same way as for a missing path, below, and a rename git recorded replaces the guess from the name.
